...
function fromWordpressButton() {
  return function wordpressButtonToCard(node, builder, _ref2) {
    let {
      addSection,
      nodeFinished
    } = _ref2;
...
function fromSubstackButton() {
  return function substackButtonToCard(node, builder, _ref3) {
    let {
      addSection,
      nodeFinished
    } = _ref3;
...
function fromMixtape(options) {
  return function mixtapeEmbed(node, builder, _ref) {
    let {
      addSection,
      nodeFinished
    } = _ref;
...
function fromFigureIframe(options) {
  return function figureIframeToEmbed(node, builder, _ref2) {
    let {
      addSection,
      nodeFinished
    } = _ref2;
...
function fromIframe() {
  return function iframeToEmbedCard(node, builder, _ref3) {
    let {
      addSection,
      nodeFinished
    } = _ref3;
...
function fromFigureBlockquote(options) {
  return function figureBlockquoteToEmbedCard(node, builder, _ref4) {
    let {
      addSection,
      nodeFinished
    } = _ref4;
...
function fromNFTEmbed() {
  return function fromNFTEmbedToEmbedCard(node, builder, _ref5) {
    let {
      addSection,
      nodeFinished
    } = _ref5;
...
function createParserPlugins() {
  ...
  function _readFigCaptionFromNode(node, payload) {
    let selector = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'figcaption';
    let figcaptions = Array.from(node.querySelectorAll(selector));
  ...
  function kgCalloutCardToCard(node, builder, _ref) {
    let {
      addSection,
      nodeFinished
    } = _ref;
  ...
  return [fromNFTEmbed(), fromMixtape(options), fromKoenigCard(), fromKoenigCard$1(), fromWordpressButton(), fromSubstackButton(), kgCalloutCardToCard, kgToggleCardToCard, blockquoteWithChildren, fromBr(), removeLeadingNewline, kgGalleryCardToCard, fromFigureBlockquote(options), // I think these can contain images
